...
import { UserRole, BookingStatus, PujaType } from '@prisma/client';
...
export class PanditService {
  ...
  async updateBookingStatus(bookingId: string, status: BookingStatus, currentUser: UserContext) {
    // Get the booking
    ...
    // - Pandit can update their own bookings to any status
    // - User can only cancel their own bookings
    const userRole = currentUser.role;
    const isAdmin = userRole === UserRole.ADMIN || userRole === UserRole.SUPER_ADMIN;
    const isUser = userRole === UserRole.USER;
    const isPandit = userRole === UserRole.PANDIT;
    
    // Check if user is trying to cancel (users can only cancel their own bookings)
    const isCancelling = status === BookingStatus.CANCELLED;
    const isOwnBooking = booking.userId === currentUser.userId;
    
    if (isAdmin) {
      // Admin can update any booking - proceed with update
    } else if (isUser) {
      // Users can only cancel their own bookings
      if (!isCancelling) {
        throw new ForbiddenException('Users can only cancel their own bookings');
      }
      if (!isOwnBooking) {
        throw new ForbiddenException('You can only cancel your own bookings');
      }
    } else if (isPandit) {
      // Pandits can update their own bookings
      const pandit = await this.prisma.pandit.findUnique({
        where: { userId: currentUser.userId },
      });

      if (!pandit) {
        throw new ForbiddenException('Pandit profile not found');
      }

      if (booking.panditId !== pandit.id) {
    ...
    const statusMessages: Record<BookingStatus, { title: string; message: string }> = {
      ...
      CONFIRMED: { title: 'Booking Confirmed', message: 'Your booking has been confirmed by the pandit' },
      IN_PROGRESS: { title: 'Service Started', message: 'The pandit has started your service' },
      COMPLETED: { title: 'Service Completed', message: 'Your service has been completed. Please rate and review.' },
      CANCELLED: { title: 'Booking Cancelled', message: 'Your booking has been cancelled' },
      NO_SHOW: { title: 'No Show', message: 'You were marked as no show for this booking' },
    };
  ...
  async getPanditAvailability(panditId: string, date?: string) {
    ...
    // Collect all booked times, including adjacent hours for offline bookings
    const bookedTimes = new Set<string>();
    existingBookings.forEach(booking => {
      bookedTimes.add(booking.bookingTime);
      
      // For offline bookings, also block previous 1 hour and next 1 hour
      if (booking.pujaType === PujaType.OFFLINE) {
        const previousHour = addHours(booking.bookingTime, -1);
        const nextHour = addHours(booking.bookingTime, 1);
        bookedTimes.add(previousHour);
        bookedTimes.add(nextHour);
      }
    });
